...
# I worked with Nick Rubera on this, which is why the variable names and syntax might be similar.
# He wrote out the script while I verbally stated what values should be according to the pdf.
# We figured this would be faster than both of us having to type it individually and helped when we ran into
# errors, as two sets of eyes are better than one.
...
check "Password Warn Age" "i7" "${pwarn}"
...
chkIPforward=$(egrep -i '^net.ipv4.ip_forward' /etc/sysctl.conf | awk ' { print $2 } ' )

check "IP Forwarding" "0" "${chkIPforward}" "Edit /etc/sysctl.conf and set: \nnet.ipv4.ip_forward=1\nto\nnet.ipv4.ip_forward=0.\nThen run: \n sysctl -w"


#Check that ICMP Redirecting is disabled

chkICMPred=$(egrep -i '^net.ipv4.conf.all.accept_redirects' /etc/sysctl.conf | awk ' { print $2 } ' )

check "ICMP Redirects" "0" "${chkICMPred}"
...
check "UID 0" "root:x:0:0:root:/root:usr/bin/zsh" "${uid0}" "This is an error, don't really know why it's happening"
